...
- 快捷短语（K）、日期时间、Unicode（U）、Emoji（E）、颜文字（M）、超级简拼（J）、临时英文（Y）、临时日语（R）。
...
均在中文状态下用 `Shift + 字母` 进入，可在设置 → 实用功能中分别开关；日期时间为直接输入，无 `Shift` 前缀。

| 模式 | 快捷键 | 说明 |
|---|---|---|
| 快捷短语（K） | `Shift + K` | 再输入字母编码调用短语。编码只能是英文字母。支持查询、新增、批量导入和导出。 |
| 日期时间 | 无前缀 | 全拼下直接输入 `rq` / `r'q` / `riqi` / `ri'qi` 得到当前日期，`sj` / `s'j` / `shijian` / `shi'jian` 得到当前时间，`xq` / `xingqi` / `week` 得到星期。候选固定出现在第 2 位，与中英混输一致；主 / 副候选格式（默认跟随系统长 / 短格式）在设置 → 实用功能中调整。 |
